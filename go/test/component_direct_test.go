...
func TestComponentDirect(t *testing.T) {
	t.Run("direct-list-component", func(t *testing.T) {
		setup := componentDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-component", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"page_access_group01", "page01"} {
	...
	t.Run("direct-load-component", func(t *testing.T) {
		setup := componentDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-component", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["page_access_group_id"] = setup.idmap["page_access_group01"]
...
type componentDirectSetupResult struct {
...
func componentDirectSetup(mockres any) *componentDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"STATUSPAGE_TEST_COMPONENT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["STATUSPAGE_TEST_COMPONENT_ENTID"]; ok {
		...
		return &componentDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &componentDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
