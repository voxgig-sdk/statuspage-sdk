...
func TestPageAccessGroupEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.PageAccessGroup(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.PageAccessGroup(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		pageAccessGroupRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "page_access_group"}), "page_access_group_ref01"))
		...
		pageAccessGroupRef01Data = core.ToMapAny(entityData(pageAccessGroupRef01DataResult))
		...
		pageAccessGroupRef01ResdataUp0 := core.ToMapAny(entityData(pageAccessGroupRef01ResdataUp0Result))
		...
		pageAccessGroupRef01DataDt0LoadResult := core.ToMapAny(entityData(pageAccessGroupRef01DataDt0Loaded))
