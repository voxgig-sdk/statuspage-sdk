...
func TestMetricDirect(t *testing.T) {
	t.Run("direct-list-metric", func(t *testing.T) {
		setup := metricDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-metric", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"page_access_user01", "page01"} {
	...
	t.Run("direct-load-metric", func(t *testing.T) {
		setup := metricDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-metric", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["page_access_user_id"] = setup.idmap["page_access_user01"]
...
type metricDirectSetupResult struct {
...
func metricDirectSetup(mockres any) *metricDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"STATUSPAGE_TEST_METRIC_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["STATUSPAGE_TEST_METRIC_ENTID"]; ok {
		...
		return &metricDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &metricDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
