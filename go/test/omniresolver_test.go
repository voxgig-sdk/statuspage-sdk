...
import (
	...
	omni "github.com/voxgig-sdk/statuspage-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StatuspageError); is {
				out := map[string]any{
					"name":    "StatuspageError",
...
func omniCtx(arg any, client *sdk.StatuspageSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StatuspageSDK); is {
			client = live
