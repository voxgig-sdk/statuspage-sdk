...
func TestSubscriberDirect(t *testing.T) {
	t.Run("direct-list-subscriber", func(t *testing.T) {
		setup := subscriberDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-subscriber", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-subscriber", func(t *testing.T) {
		setup := subscriberDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-subscriber", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type subscriberDirectSetupResult struct {
...
func subscriberDirectSetup(mockres any) *subscriberDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"STATUSPAGE_TEST_SUBSCRIBER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["STATUSPAGE_TEST_SUBSCRIBER_ENTID"]; ok {
		...
		return &subscriberDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &subscriberDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
