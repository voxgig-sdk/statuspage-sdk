// Command Statuspage-mcp is an MCP server exposing the Statuspage
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Statuspage_list  — list records of an entity
//	Statuspage_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: STATUSPAGE_APIKEY carries the API key and
	// STATUSPAGE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STATUSPAGE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STATUSPAGE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStatuspageSDK(opts)
