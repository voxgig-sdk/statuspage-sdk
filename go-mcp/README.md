...
[MCP](https://modelcontextprotocol.io) server exposing the Statuspage SDK as
two agent tools — `statuspage_list` and `statuspage_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/statuspage-mcp)
make build

# 2. Provide credentials via the environment
export STATUSPAGE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user statuspage \
  -- /absolute/path/to/statuspage-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./statuspage-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// statuspage_list: first page of records
{ "entity": "component" }
{ "entity": "component", "query": { } }

// statuspage_load: one record by id
{ "entity": "component", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/statuspage-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user statuspage \
     -- "$PWD"/dist/*/statuspage-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `statuspage_list` and `statuspage_load` tools now appear
   in new sessions. Ask the agent to *"list component using statuspage"*
   and it calls `statuspage_list` with `{"entity":"component"}`.
...
```sh
export STATUSPAGE_APIKEY=sk_live_xxx            # API key
export STATUSPAGE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `statuspage_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `statuspage_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STATUSPAGE_APIKEY` | API key sent with every request. |
| `STATUSPAGE_BASE` | Optional override of the API base URL. |
...
```sh
./statuspage-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"statuspage_load","arguments":{"entity":"component","query":{"id":1}}}}'
```
