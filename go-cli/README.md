...
boru-driven command-line client **and** interactive REPL for the Statuspage
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/statuspage-cli)
make build
...
./statuspage-cli list component
./statuspage-cli load 1 component            # {id:1} shorthand
./statuspage-cli load '{id:1}' component       # explicit match map
./statuspage-cli update '{name:"x"}' component
./statuspage-cli list component_group_uptime

# 5. Override the API base URL for a single call
STATUSPAGE_BASE=https://api.example.com ./statuspage-cli list component

# 6. No arguments -> interactive REPL
./statuspage-cli
statuspage> list component
statuspage> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/statuspage-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/statuspage-cli list component
   ```
...
```sh
./statuspage-cli load 1 component          # scalar shorthand for {id:1}
./statuspage-cli load '{id:1}' component     # explicit match map
```
...
```sh
./statuspage-cli update '{id:1,name:"new"}' component
```
...
Run with no arguments to open a REPL (prompt `statuspage>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `component`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/statuspage-cli`. |
...
(`./statuspage-cli <expr>`) and interactively (the REPL), and expressions compose the
