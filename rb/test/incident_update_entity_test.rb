...
class IncidentUpdateEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = incident_update_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["update"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "incident_update." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_INCIDENT_UPDATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    incident_update_ref01_data = nil
    if incident_update_ref01_data_raw.length > 0
      incident_update_ref01_data = Helpers.to_map(incident_update_ref01_data_raw[0][1])
    end

    # UPDATE
    incident_update_ref01_ent = client.IncidentUpdate(nil)
...
def incident_update_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "incident_update", "IncidentUpdateTestData.json")
  ...
  entid_env_raw = ENV["STATUSPAGE_TEST_INCIDENT_UPDATE_ENTID"]
  ...
  if idmap_resolved["incident_id"].nil?
