...
class MetricsProviderDirectTest < Minitest::Test
  def test_direct_list_metrics_provider
    setup = metrics_provider_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-metrics_provider", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["page01"].each do |_live_key|
  ...
  def test_direct_load_metrics_provider
    setup = metrics_provider_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-metrics_provider", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def metrics_provider_direct_setup(mockres)
