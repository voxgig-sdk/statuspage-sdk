...
class PostmortemDirectTest < Minitest::Test
  def test_direct_load_postmortem
    setup = postmortem_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-postmortem", setup[:live] ? "live" : "unit")
    if _should_skip
...
def postmortem_direct_setup(mockres)
