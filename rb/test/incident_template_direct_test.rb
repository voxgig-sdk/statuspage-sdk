...
class IncidentTemplateDirectTest < Minitest::Test
  def test_direct_list_incident_template
    setup = incident_template_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-incident_template", setup[:live] ? "live" : "unit")
    if _should_skip
...
def incident_template_direct_setup(mockres)
