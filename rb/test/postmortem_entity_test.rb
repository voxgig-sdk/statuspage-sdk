...
class PostmortemEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = postmortem_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "postmortem." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_POSTMORTEM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    postmortem_ref01_data = nil
    if postmortem_ref01_data_raw.length > 0
      postmortem_ref01_data = Helpers.to_map(postmortem_ref01_data_raw[0][1])
    end

    # UPDATE
    postmortem_ref01_ent = client.Postmortem(nil)
    ...
    postmortem_ref01_data_dt0_loaded = postmortem_ref01_ent.load(postmortem_ref01_match_dt0, nil)
    assert !postmortem_ref01_data_dt0_loaded.nil?

  end
end

def postmortem_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "postmortem", "PostmortemTestData.json")
  ...
  entid_env_raw = ENV["STATUSPAGE_TEST_POSTMORTEM_ENTID"]
