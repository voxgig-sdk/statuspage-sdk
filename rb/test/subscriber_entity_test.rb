...
class SubscriberEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Subscriber(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Subscriber(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = subscriber_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "subscriber." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_SUBSCRIBER_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    subscriber_ref01_ent = client.Subscriber(nil)
    ...
    subscriber_ref01_list_result = subscriber_ref01_ent.list(subscriber_ref01_match, nil)
    assert subscriber_ref01_list_result.is_a?(Array)
    ...
    subscriber_ref01_ent.remove(subscriber_ref01_match_rm0, nil)
    ...
    subscriber_ref01_list_rt0_result = subscriber_ref01_ent.list(subscriber_ref01_match_rt0, nil)
    assert subscriber_ref01_list_rt0_result.is_a?(Array)
...
def subscriber_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "subscriber", "SubscriberTestData.json")
  ...
  idmap = Vs.transform(
    ["subscriber01", "subscriber02", "subscriber03", "page01", "page02", "page03", "incident01", "incident02", "incident03"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["STATUSPAGE_TEST_SUBSCRIBER_ENTID"]
