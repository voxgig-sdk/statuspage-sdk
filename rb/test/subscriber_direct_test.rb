...
class SubscriberDirectTest < Minitest::Test
  def test_direct_list_subscriber
    setup = subscriber_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-subscriber", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_subscriber
    setup = subscriber_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-subscriber", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def subscriber_direct_setup(mockres)
