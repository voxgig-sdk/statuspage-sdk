...
class GroupComponentDirectTest < Minitest::Test
  def test_direct_list_group_component
    setup = group_component_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-group_component", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_group_component
    setup = group_component_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-group_component", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def group_component_direct_setup(mockres)
