...
module StatuspageUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StatuspageResponse.new({})
    ...
    if fetch_err
      ...
      response = StatuspageResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StatuspageResponse.new(fetched)
    else
