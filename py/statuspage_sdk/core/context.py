...
from statuspage_sdk.core.helpers import get_ctx_prop, to_map
...
class StatuspageContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, StatuspageSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, StatuspageResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, StatuspageResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return StatuspageError(code, msg, self)
