...
from statuspage_sdk.statuspage_types import (
    PageAccessGroup,
...
class PageAccessGroupEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PageAccessGroupEntity(self._client, opts)
    ...
    def data_get(self) -> PageAccessGroup:
    ...
    def match_get(self) -> PageAccessGroup:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PageAccessGroup:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PageAccessGroup().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PageAccessGroup]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PageAccessGroup().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: PageAccessGroupCreateData, ctrl=None) -> PageAccessGroup:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: PageAccessGroupUpdateData, ctrl=None) -> PageAccessGroup:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PageAccessGroup:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PageAccessGroup().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
