...
class IncidentPostmortemEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IncidentPostmortemEntity(self._client, opts)
    ...
    def data_get(self) -> IncidentPostmortem:
    ...
    def match_get(self) -> IncidentPostmortem:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> IncidentPostmortem:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.IncidentPostmortem().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
