...
class StatusEmbedConfigEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return StatusEmbedConfigEntity(self._client, opts)
    ...
    def data_get(self) -> StatusEmbedConfig:
    ...
    def match_get(self) -> StatusEmbedConfig:
    ...
    def load(self, reqmatch=None, ctrl=None) -> StatusEmbedConfig:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.StatusEmbedConfig().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: StatusEmbedConfigUpdateData, ctrl=None) -> StatusEmbedConfig:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
