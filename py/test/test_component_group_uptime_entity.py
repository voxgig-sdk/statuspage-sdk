...
class TestComponentGroupUptimeEntity:

    def test_should_create_instance(self):
        testsdk = StatuspageSDK.test(None, None)
        ent = testsdk.ComponentGroupUptime(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _component_group_uptime_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "component_group_uptime." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STATUSPAGE_TEST_COMPONENT_GROUP_UPTIME_ENTID JSON to run live")
        client = setup["client"]
        ...
        component_group_uptime_ref01_data = None
        if len(component_group_uptime_ref01_data_raw) > 0:
...
def _component_group_uptime_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/component_group_uptime/ComponentGroupUptimeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("STATUSPAGE_TEST_COMPONENT_GROUP_UPTIME_ENTID"))
