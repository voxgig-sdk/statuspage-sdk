...
class TestStatusEmbedConfigDirect:

    def test_should_direct_load_status_embed_config(self):
        setup = _status_embed_config_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-status_embed_config", "live" if setup["live"] else "unit")
        if _skip:
...
def _status_embed_config_direct_setup(mockres):
