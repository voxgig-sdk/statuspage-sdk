...
class TestSubscriberDirect:

    def test_should_direct_list_subscriber(self):
        setup = _subscriber_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-subscriber", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_subscriber(self):
        setup = _subscriber_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-subscriber", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _subscriber_direct_setup(mockres):
