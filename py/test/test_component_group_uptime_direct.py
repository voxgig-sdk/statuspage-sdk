...
class TestComponentGroupUptimeDirect:

    def test_should_direct_load_component_group_uptime(self):
        setup = _component_group_uptime_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-component_group_uptime", "live" if setup["live"] else "unit")
        if _skip:
...
def _component_group_uptime_direct_setup(mockres):
