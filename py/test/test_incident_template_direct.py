...
class TestIncidentTemplateDirect:

    def test_should_direct_list_incident_template(self):
        setup = _incident_template_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-incident_template", "live" if setup["live"] else "unit")
        if _skip:
...
def _incident_template_direct_setup(mockres):
