...
class TestSubscriberEntity:

    def test_should_create_instance(self):
        testsdk = StatuspageSDK.test(None, None)
        ent = testsdk.Subscriber(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Subscriber(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Subscriber(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _subscriber_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "subscriber." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STATUSPAGE_TEST_SUBSCRIBER_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        subscriber_ref01_ent = client.Subscriber(None)
        ...
        subscriber_ref01_list_result = subscriber_ref01_ent.list(subscriber_ref01_match, None)
        assert isinstance(subscriber_ref01_list_result, list)
        ...
        subscriber_ref01_ent.remove(subscriber_ref01_match_rm0, None)
        ...
        subscriber_ref01_list_rt0_result = subscriber_ref01_ent.list(subscriber_ref01_match_rt0, None)
        assert isinstance(subscriber_ref01_list_rt0_result, list)
...
def _subscriber_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/subscriber/SubscriberTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("STATUSPAGE_TEST_SUBSCRIBER_ENTID"))
