...
class TestMetricsProviderDirect:

    def test_should_direct_list_metrics_provider(self):
        setup = _metrics_provider_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-metrics_provider", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["page01"]:
    ...
    def test_should_direct_load_metrics_provider(self):
        setup = _metrics_provider_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-metrics_provider", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _metrics_provider_direct_setup(mockres):
