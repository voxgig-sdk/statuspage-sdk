...
// Statuspage SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = StatuspageOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(StatuspageOmni::structfix($now, Struct::undef()))
                        !== json_encode(StatuspageOmni::structfix($val, Struct::undef()))) {
