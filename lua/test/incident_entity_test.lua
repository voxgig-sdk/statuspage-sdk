...
describe("IncidentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Incident(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Incident(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Incident(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = incident_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "incident." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_INCIDENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local incident_ref01_ent = client:Incident(nil)
    local incident_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local incident_ref01_match = {
      ["page_id"] = setup.idmap["page01"],
    }

    local incident_ref01_list_result, err = incident_ref01_ent:list(incident_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(incident_ref01_list_result)
    ...
    -- UPDATE
    local incident_ref01_data_up0_up = {
      id = incident_ref01_data["id"],
      ["page_id"] = setup.idmap["page_id"],
    }

    local incident_ref01_markdef_up0_name = "created_at"
    local incident_ref01_markdef_up0_value = "Mark01-incident_ref01_" .. tostring(setup.now)
    ...
    -- LOAD
    local incident_ref01_match_dt0 = {
      id = incident_ref01_data["id"],
    }
    ...
    -- REMOVE
    local incident_ref01_match_rm0 = {
      id = incident_ref01_data["id"],
    }
    local _, err = incident_ref01_ent:remove(incident_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local incident_ref01_match_rt0 = {
      ["page_id"] = setup.idmap["page01"],
    }

    local incident_ref01_list_rt0_result, err = incident_ref01_ent:list(incident_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(incident_ref01_list_rt0_result)
...
function incident_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/incident/IncidentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read incident test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("STATUSPAGE_TEST_INCIDENT_ENTID")
