...
describe("SubscriberEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Subscriber(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_SUBSCRIBER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local subscriber_ref01_ent = client:Subscriber(nil)
    local subscriber_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LOAD
    local subscriber_ref01_match_dt0 = {
      id = subscriber_ref01_data["id"],
    }
    ...
    -- REMOVE
    local subscriber_ref01_match_rm0 = {
      id = subscriber_ref01_data["id"],
    }
...
function subscriber_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/subscriber/SubscriberTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read subscriber test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("STATUSPAGE_TEST_SUBSCRIBER_ENTID")
