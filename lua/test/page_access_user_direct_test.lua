...
describe("PageAccessUserDirect", function()
  it("should direct-list-page_access_user", function()
    local setup = page_access_user_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-page_access_user", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"page_access_user01"}) do
  ...
  it("should direct-load-page_access_user", function()
    local setup = page_access_user_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-page_access_user", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function page_access_user_direct_setup(mockres)
