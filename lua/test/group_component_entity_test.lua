...
describe("GroupComponentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:GroupComponent(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:GroupComponent(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:GroupComponent(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = group_component_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "group_component." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_GROUP_COMPONENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local group_component_ref01_ent = client:GroupComponent(nil)
    local group_component_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local group_component_ref01_match = {
      ["page_id"] = setup.idmap["page01"],
    }

    local group_component_ref01_list_result, err = group_component_ref01_ent:list(group_component_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(group_component_ref01_list_result)
    ...
    -- UPDATE
    local group_component_ref01_data_up0_up = {
      id = group_component_ref01_data["id"],
    ...
    -- LOAD
    local group_component_ref01_match_dt0 = {
      id = group_component_ref01_data["id"],
    }
    ...
    -- REMOVE
    local group_component_ref01_match_rm0 = {
      id = group_component_ref01_data["id"],
    }
    local _, err = group_component_ref01_ent:remove(group_component_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local group_component_ref01_match_rt0 = {
      ["page_id"] = setup.idmap["page01"],
    }

    local group_component_ref01_list_rt0_result, err = group_component_ref01_ent:list(group_component_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(group_component_ref01_list_rt0_result)
...
function group_component_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/group_component/GroupComponentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read group_component test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("STATUSPAGE_TEST_GROUP_COMPONENT_ENTID")
