...
describe("PageEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Page(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Page(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_PAGE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local page_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.page")))
    local page_ref01_data = nil
    if #page_ref01_data_raw > 0 then
      page_ref01_data = helpers.to_map(page_ref01_data_raw[1][2])
    end

    -- LIST
    local page_ref01_ent = client:Page(nil)
    local page_ref01_match = {}

    local page_ref01_list_result, err = page_ref01_ent:list(page_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(page_ref01_list_result)

    -- UPDATE
    local page_ref01_data_up0_up = {
      id = page_ref01_data["id"],
    }

    local page_ref01_markdef_up0_name = "branding"
    local page_ref01_markdef_up0_value = "Mark01-page_ref01_" .. tostring(setup.now)
...
function page_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("STATUSPAGE_TEST_PAGE_ENTID")
