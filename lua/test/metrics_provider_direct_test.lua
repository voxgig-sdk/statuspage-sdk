...
describe("MetricsProviderDirect", function()
  it("should direct-list-metrics_provider", function()
    local setup = metrics_provider_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-metrics_provider", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"page01"}) do
  ...
  it("should direct-load-metrics_provider", function()
    local setup = metrics_provider_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-metrics_provider", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function metrics_provider_direct_setup(mockres)
