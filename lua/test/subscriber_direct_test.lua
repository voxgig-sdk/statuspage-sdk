...
describe("SubscriberDirect", function()
  it("should direct-list-subscriber", function()
    local setup = subscriber_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-subscriber", setup.live and "live" or "unit")
  ...
  it("should direct-load-subscriber", function()
    local setup = subscriber_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-subscriber", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function subscriber_direct_setup(mockres)
