...
describe("IncidentDirect", function()
  it("should direct-list-incident", function()
    local setup = incident_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-incident", setup.live and "live" or "unit")
  ...
  it("should direct-load-incident", function()
    local setup = incident_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-incident", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function incident_direct_setup(mockres)
