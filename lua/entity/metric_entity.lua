...
local MetricEntity = {}
MetricEntity.__index = MetricEntity


function MetricEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MetricEntity)
  self._name = "metric"
  self._client = client
...
function MetricEntity:get_name()
  return self._name
end


function MetricEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MetricEntity.new(self._client, opts)
end
...
function MetricEntity:mark_deleted()
  self._deleted = true
end


function MetricEntity:deleted()
  return true == self._deleted
end


function MetricEntity:data_set(args)
  if args ~= nil then
...
function MetricEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MetricEntity:match_set(args)
  if args ~= nil then
...
function MetricEntity:match_get()
...
function MetricEntity:stream(action, args, callopts)
...
---@param reqmatch MetricListMatch
---@param ctrl? table
---@return Metric[]
---@return string? err
function MetricEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata MetricCreateData
---@param ctrl? table
---@return Metric
---@return string? err
function MetricEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata MetricUpdateData
---@param ctrl? table
---@return Metric
---@return string? err
function MetricEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch MetricRemoveMatch
---@param ctrl? table
---@return Metric
---@return string? err
function MetricEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function MetricEntity:_run_op(ctx, post_done)
  local utility = self._utility
