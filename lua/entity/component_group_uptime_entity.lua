...
local ComponentGroupUptimeEntity = {}
ComponentGroupUptimeEntity.__index = ComponentGroupUptimeEntity


function ComponentGroupUptimeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ComponentGroupUptimeEntity)
  self._name = "component_group_uptime"
  self._client = client
...
function ComponentGroupUptimeEntity:get_name()
  return self._name
end


function ComponentGroupUptimeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ComponentGroupUptimeEntity.new(self._client, opts)
end
...
function ComponentGroupUptimeEntity:mark_deleted()
  self._deleted = true
end


function ComponentGroupUptimeEntity:deleted()
  return true == self._deleted
end


function ComponentGroupUptimeEntity:data_set(args)
  if args ~= nil then
...
function ComponentGroupUptimeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ComponentGroupUptimeEntity:match_set(args)
  if args ~= nil then
...
function ComponentGroupUptimeEntity:match_get()
...
function ComponentGroupUptimeEntity:stream(action, args, callopts)
...
---@param reqmatch ComponentGroupUptimeLoadMatch
---@param ctrl? table
---@return ComponentGroupUptime
---@return string? err
function ComponentGroupUptimeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ComponentGroupUptimeEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ComponentGroupUptimeEntity
