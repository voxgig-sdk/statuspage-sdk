...
describe('PostmortemDirect', async () => {
  ...
  test('direct-load-postmortem', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-postmortem', setup.live)) return
    if (skipIfMissingIds(t, setup, ["incident_id01","page_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_POSTMORTEM_ENTID']
