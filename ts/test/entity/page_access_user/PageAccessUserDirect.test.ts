...
describe('PageAccessUserDirect', async () => {
  ...
  test('direct-load-page_access_user', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-page_access_user', setup.live)) return
  ...
  test('direct-list-page_access_user', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-page_access_user', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page_access_user01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_PAGE_ACCESS_USER_ENTID']
