...
describe('PermissionDirect', async () => {
  ...
  test('direct-load-permission', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-permission', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","organization_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_PERMISSION_ENTID']
