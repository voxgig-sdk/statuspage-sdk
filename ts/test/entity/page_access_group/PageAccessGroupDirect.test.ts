...
describe('PageAccessGroupDirect', async () => {
  ...
  test('direct-load-page_access_group', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-page_access_group', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page01"])) return
  ...
  test('direct-list-page_access_group', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-page_access_group', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page_access_group01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_PAGE_ACCESS_GROUP_ENTID']
