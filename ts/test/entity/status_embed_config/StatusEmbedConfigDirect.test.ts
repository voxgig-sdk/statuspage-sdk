...
describe('StatusEmbedConfigDirect', async () => {
  ...
  test('direct-load-status_embed_config', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-status_embed_config', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_STATUS_EMBED_CONFIG_ENTID']
