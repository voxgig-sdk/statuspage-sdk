...
describe('MetricDirect', async () => {
  ...
  test('direct-load-metric', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-metric', setup.live)) return
    if (skipIfMissingIds(t, setup, ["metrics_provider01","page01"])) return
  ...
  test('direct-list-metric', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-metric', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page_access_user01","page01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_METRIC_ENTID']
