...
describe('IncidentDirect', async () => {
  ...
  test('direct-load-incident', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-incident', setup.live)) return
  ...
  test('direct-list-incident', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-incident', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_INCIDENT_ENTID']
