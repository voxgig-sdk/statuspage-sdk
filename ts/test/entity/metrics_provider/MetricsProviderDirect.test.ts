...
describe('MetricsProviderDirect', async () => {
  ...
  test('direct-load-metrics_provider', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-metrics_provider', setup.live)) return
  ...
  test('direct-list-metrics_provider', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-metrics_provider', setup.live)) return
    if (skipIfMissingIds(t, setup, ["page01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_METRICS_PROVIDER_ENTID']
