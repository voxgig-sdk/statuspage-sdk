...
describe('SubscriberDirect', async () => {
  ...
  test('direct-load-subscriber', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-subscriber', setup.live)) return
    if (skipIfMissingIds(t, setup, ["incident01","page01"])) return
  ...
  test('direct-list-subscriber', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-subscriber', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_SUBSCRIBER_ENTID']
