...
describe('ComponentGroupUptimeDirect', async () => {
  ...
  test('direct-load-component_group_uptime', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-component_group_uptime', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","page_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STATUSPAGE_TEST_COMPONENT_GROUP_UPTIME_ENTID']
