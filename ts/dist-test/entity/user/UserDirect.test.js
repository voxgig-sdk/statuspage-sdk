...
(0, node_test_1.describe)('UserDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-user', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-user', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["organization01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.organization_id = setup.idmap['organization01'];
        }
        else {
            params.organization_id = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['STATUSPAGE_TEST_USER_ENTID'];
