...
(0, node_test_1.describe)('PermissionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-permission', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-permission', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["id01", "organization_id01"]))
            return;
        ...
        if (setup.live) {
            ...
            params.id = 'direct01';
            params.organization_id = 'direct02';
        }
        const result = await client.direct({
            path: 'organizations/{organization_id}/permissions/{id}',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['STATUSPAGE_TEST_PERMISSION_ENTID'];
