...
(0, node_test_1.describe)('PageAccessGroupDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.StatuspageSDK({
    ...
    (0, node_test_1.test)('direct-load-page_access_group', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-page_access_group', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["page01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                ...
                method: 'GET',
                params: {
                    id: setup.idmap['page_access_group01'],
                },
            });
    ...
    (0, node_test_1.test)('direct-list-page_access_group', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-page_access_group', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["page_access_group01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.STATUSPAGE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.StatuspageSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.STATUSPAGE_APIKEY,
        }));
        let idmap = env['STATUSPAGE_TEST_PAGE_ACCESS_GROUP_ENTID'];
    ...
    const client = new __1.StatuspageSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=PageAccessGroupDirect.test.js.map