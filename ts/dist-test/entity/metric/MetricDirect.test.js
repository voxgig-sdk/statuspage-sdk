...
(0, node_test_1.describe)('MetricDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-metric', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-metric', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["metrics_provider01", "page01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.metrics_provider_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.metrics_provider_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-metric', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-metric', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["page_access_user01", "page01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
//# sourceMappingURL=MetricDirect.test.js.map