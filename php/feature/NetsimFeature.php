...
class StatuspageNetsimFeature extends StatuspageBaseFeature
{
    ...
    private function _simulate(StatuspageContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(StatuspageContext $ctx, array $applied): void
    {
        $client = $this->client;
