...
class FtCtrl extends StatuspageControl
{
...
class FtClient
{
    ...
    private StatuspageUtility $utility;

    public function __construct(StatuspageUtility $utility, array $options)
    {
    ...
    public function get_utility(): StatuspageUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StatuspageContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StatuspageContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StatuspageContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StatuspageConfig::make_feature($name);
    ...
    private static function buildUrl(StatuspageSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StatuspageError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StatuspageResponse($fetched);
        }
    ...
    private function fail(StatuspageContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StatuspageContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
