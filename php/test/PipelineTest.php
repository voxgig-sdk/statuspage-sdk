...
{
    private static function utility(): StatuspageUtility
    {
        return new StatuspageUtility();
    }
    ...
    private static function ctx(array $over = []): StatuspageContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new StatuspageContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new StatuspageOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof StatuspageError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new StatuspageOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = StatuspageMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new StatuspageOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = StatuspageMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new StatuspageOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = StatuspageMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StatuspageSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = StatuspageMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new StatuspageResult([]),
        ]);
        [, $err] = StatuspageMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new StatuspageSpec([]),
            'response' => null,
            'result' => new StatuspageResult([]),
        ]);
        [, $err] = StatuspageMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new StatuspageSpec(['step' => 's']),
            'response' => new StatuspageResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new StatuspageResult([]),
        ]);
        [, $err] = StatuspageMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new StatuspageSpec(['step' => 's']),
            'response' => new StatuspageResponse(self::resp(200, ['v' => 1])),
            'result' => new StatuspageResult([]),
        ]);
        [, $err] = StatuspageMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new StatuspageSpec(['step' => 's']),
            'response' => new StatuspageResponse(self::resp(200, ['v' => 2])),
            'result' => new StatuspageResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        StatuspageMakeResponse::call($ctx);
    ...
    {
        $preset = new StatuspageResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new StatuspageSpec([]),
            'response' => new StatuspageResponse([]),
            'result' => new StatuspageResult([]),
        ...
        [$got, $err] = StatuspageMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new StatuspageResult([])]);
        [, $err] = StatuspageMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new StatuspageSpec([]), 'result' => null]);
        [, $err] = StatuspageMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StatuspageSpec(['step' => 's']),
            'result' => new StatuspageResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new StatuspageOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StatuspageMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StatuspageSpec(['step' => 's']),
            'result' => new StatuspageResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new StatuspageOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StatuspageMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StatuspageResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new StatuspageSpec([]),
            'result' => new StatuspageResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = StatuspageMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StatuspageMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new StatuspageError('boom', 'boom');
        $utility->fetcher = function (StatuspageContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StatuspageSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = StatuspageMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (StatuspageContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = StatuspageMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(StatuspageResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        StatuspageMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (StatuspageContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StatuspageSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = StatuspageMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new StatuspageResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new StatuspageSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = StatuspageMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = StatuspageMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = StatuspageMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new StatuspageResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, StatuspageDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new StatuspageResult(['ok' => false])]);
        $this->expectException(StatuspageError::class);
        StatuspageDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new StatuspageResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', StatuspageMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        StatuspageMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new StatuspageBaseFeature();
        $b = new StatuspageBaseFeature();
        StatuspageFeatureAdd::call($ctx, $a);
        StatuspageFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): StatuspageBaseFeature
    {
        $f = new StatuspageBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        StatuspageFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): StatuspageContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new StatuspageSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StatuspagePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        StatuspagePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new StatuspageResponse(['headers' => null]),
            'result' => new StatuspageResult([]),
        ]);
        StatuspageResultHeaders::call($ctx);
