...
class PostmortemEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = StatuspageSDK::test(null, null);
        $ent = $testsdk->Postmortem(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = postmortem_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "postmortem." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STATUSPAGE_TEST_POSTMORTEM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $postmortem_ref01_data = null;
        if (count($postmortem_ref01_data_raw) > 0) {
            $postmortem_ref01_data = Helpers::to_map($postmortem_ref01_data_raw[0][1]);
        }

        // UPDATE
        $postmortem_ref01_ent = $client->Postmortem(null);
        $postmortem_ref01_data_up0_up = [
            "page_id" => $setup["idmap"]["page_id"],
        ];

        $postmortem_ref01_markdef_up0_name = "body";
        $postmortem_ref01_markdef_up0_value = "Mark01-postmortem_ref01_" . $setup["now"];
        ...
        $postmortem_ref01_data_dt0_loaded = $postmortem_ref01_ent->load($postmortem_ref01_match_dt0, null);
        $this->assertNotNull($postmortem_ref01_data_dt0_loaded);
...
function postmortem_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/postmortem/PostmortemTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["postmortem01", "postmortem02", "postmortem03", "page01", "page02", "page03", "incident01", "incident02", "incident03"] as $k) {
    ...
    $entid_env_raw = getenv("STATUSPAGE_TEST_POSTMORTEM_ENTID");
